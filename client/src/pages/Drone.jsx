...
import { Drone as DroneIcon, Battery, Activity, Calendar, Plus, Clock, Ruler, ImageIcon, Stethoscope, MapPin, Zap } from 'lucide-react';
...
export default function DronePage() {
  ...
  const getBatteryColor = (level) => {
    if (level >= 70) return 'var(--status-healthy)';
    if (level >= 40) return 'var(--status-moderate)';
    return 'var(--status-poor)';
  };

  const getHealthColor = (status) => {
    if (status === 'Excellent' || status === 'Good') return 'var(--status-healthy)';
    if (status === 'Fair') return 'var(--status-moderate)';
    return 'var(--status-poor)';
  };
      ...
      <div className="container">
        <div className="card">
          <div style={{ textAlign: 'center', padding: 'var(--space-12)', color: 'var(--text-muted)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 'var(--space-4)' }}>
            <DroneIcon size={48} strokeWidth={1.5} className="animate-pulse" aria-hidden />
            <span>Loading drone information...</span>
          </div>
        </div>
      </div>
    );
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0, 0, 0, 0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 'var(--space-4)', backdropFilter: 'blur(4px)' }} onClick={() => setShowScheduleModal(false)}>
          <div className="card card-elevated" style={{ width: '100%', maxWidth: 480, maxHeight: '90vh', overflow: 'auto' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--space-5)' }}>
              ...
              <button onClick={() => setShowScheduleModal(false)} style={{ background: 'none', border: 'none', fontSize: 'var(--font-size-xl)', cursor: 'pointer', color: 'var(--text-muted)', padding: 'var(--space-1)', lineHeight: 1 }}>×</button>
            </div>
            
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
              <div>
                <label style={{ display: 'block', marginBottom: 'var(--space-2)', fontSize: 'var(--font-size-sm)', fontWeight: 'var(--font-weight-medium)', color: 'var(--text-primary)' }}>Schedule Name *</label>
                <input type="text" value={newSchedule.name} onChange={e => setNewSchedule(prev => ({ ...prev, name: e.target.value }))} placeholder="e.g., Morning Field Survey" className="input" />
              </div>

              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-3)' }}>
                <div>
                  <label style={{ display: 'block', marginBottom: 'var(--space-2)', fontSize: 'var(--font-size-sm)', fontWeight: 'var(--font-weight-medium)', color: 'var(--text-primary)' }}>Time</label>
                  <input type="time" value={newSchedule.time} onChange={e => setNewSchedule(prev => ({ ...prev, time: e.target.value }))} style={{ width: '100%', padding: 'var(--space-3)', border: '1px solid var(--bg-border)', borderRadius: 'var(--radius-md)', fontSize: 'var(--font-size-sm)', background: 'var(--bg-base)', color: 'var(--text-primary)' }} />
                </div>
                <div>
                  <label style={{ display: 'block', marginBottom: 'var(--space-2)', fontSize: 'var(--font-size-sm)', fontWeight: 'var(--font-weight-medium)', color: 'var(--text-primary)' }}>Duration (min)</label>
                  <input type="number" min="5" max="120" value={newSchedule.duration} onChange={e => setNewSchedule(prev => ({ ...prev, duration: parseInt(e.target.value) || 20 }))} style={{ width: '100%', padding: 'var(--space-3)', border: '1px solid var(--bg-border)', borderRadius: 'var(--radius-md)', fontSize: 'var(--font-size-sm)', background: 'var(--bg-base)', color: 'var(--text-primary)' }} />
                </div>
              </div>

              <div>
                <label style={{ display: 'block', marginBottom: 'var(--space-2)', fontSize: 'var(--font-size-sm)', fontWeight: 'var(--font-weight-medium)', color: 'var(--text-primary)' }}>Area</label>
                <select value={newSchedule.area} onChange={e => setNewSchedule(prev => ({ ...prev, area: e.target.value }))} style={{ width: '100%', padding: 'var(--space-3)', border: '1px solid var(--bg-border)', borderRadius: 'var(--radius-md)', fontSize: 'var(--font-size-sm)', background: 'var(--bg-base)', color: 'var(--text-primary)', cursor: 'pointer' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-3)' }}>
                <div>
                  <label style={{ display: 'block', marginBottom: 'var(--space-2)', fontSize: 'var(--font-size-sm)', fontWeight: 'var(--font-weight-medium)', color: 'var(--text-primary)' }}>Priority</label>
                  <select value={newSchedule.priority} onChange={e => setNewSchedule(prev => ({ ...prev, priority: e.target.value }))} style={{ width: '100%', padding: 'var(--space-3)', border: '1px solid var(--bg-border)', borderRadius: 'var(--radius-md)', fontSize: 'var(--font-size-sm)', background: 'var(--bg-base)', color: 'var(--text-primary)', cursor: 'pointer' }}>
                ...
                <div>
                  <label style={{ display: 'block', marginBottom: 'var(--space-2)', fontSize: 'var(--font-size-sm)', fontWeight: 'var(--font-weight-medium)', color: 'var(--text-primary)' }}>Days from now</label>
                  <input type="number" min="0" max="30" value={newSchedule.daysFromNow} onChange={e => setNewSchedule(prev => ({ ...prev, daysFromNow: parseInt(e.target.value) || 1 }))} style={{ width: '100%', padding: 'var(--space-3)', border: '1px solid var(--bg-border)', borderRadius: 'var(--radius-md)', fontSize: 'var(--font-size-sm)', background: 'var(--bg-base)', color: 'var(--text-primary)' }} />
                </div>
              </div>
      ...
      <div className="container-grid">
        ...
        <div className="card card-elevated animate-fade-in-up">
          ...
          {battery ? (
            <div>
              ...
              <div style={{ marginTop: 'var(--space-4)', padding: 'var(--space-4)', background: 'var(--bg-surface-elevated)', borderRadius: 'var(--radius-lg)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <div className="metric-label">Estimated Flight Time</div>
                  <div style={{ fontSize: 'var(--font-size-lg)', fontWeight: 'var(--font-weight-semibold)', fontFamily: 'var(--font-mono)' }}>{battery.estimatedTimeRemaining} <span className="unit">min</span></div>
                </div>
                <span className={`badge ${battery.charging ? 'badge-warning' : 'badge-info'}`} style={{ display: 'inline-flex', alignItems: 'center', gap: 'var(--space-1)' }}>
                  {battery.charging ? <Zap size={12} aria-hidden /> : <Battery size={12} aria-hidden />}
            ...
            <div className="empty-state">
              <div className="empty-state-icon"><Battery size={48} strokeWidth={1} aria-hidden /></div>
        ...
        <div className="card card-elevated animate-fade-in-up stagger-1">
          ...
          {health ? (
              ...
              <div style={{ marginBottom: 'var(--space-6)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--space-4)' }}>
                  <div>
                    ...
                    <div style={{ fontSize: 'var(--font-size-2xl)', fontWeight: 'var(--font-weight-bold)', color: getHealthColor(health.overall) }}>{health.overall}</div>
                  </div>
                  <div style={{ padding: 'var(--space-3) var(--space-5)', borderRadius: 'var(--radius-md)', background: 'var(--bg-surface)', border: '1px solid var(--bg-border)', color: 'var(--text-primary)', fontSize: 'var(--font-size-sm)', fontWeight: 'var(--font-weight-semibold)' }}>{health.status}</div>
                </div>
                <div className="metrics">
                ...
                <div className="list">
                  {Object.entries(health.components).map(([name, data]) => (
                    <div key={name} className="list-item" style={{ cursor: 'default' }}>
                      <div style={{ flex: 1 }}><div style={{ fontSize: 'var(--font-size-sm)', fontWeight: 'var(--font-weight-semibold)', textTransform: 'capitalize', marginBottom: 'var(--space-1)' }}>{name}</div><div style={{ fontSize: 'var(--font-size-xs)', color: 'var(--text-muted)' }}>{data.hours} hours</div></div>
            ...
            <div className="empty-state">
              <div className="empty-state-icon"><Stethoscope size={48} strokeWidth={1} aria-hidden /></div>
        ...
        <div className="card card-elevated animate-fade-in-up stagger-2">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--space-4)' }}>
            ...
            <button className="btn btn-primary btn-sm" style={{ display: 'inline-flex', alignItems: 'center', gap: 'var(--space-2)' }} onClick={() => setShowScheduleModal(true)}>
          ...
          {schedules.length > 0 ? (
            <div className="list">
              {schedules.map((schedule) => (
                <div key={schedule.id} className="list-item">
                  <div style={{ flex: 1 }}><div style={{ fontSize: 'var(--font-size-sm)', fontWeight: 'var(--font-weight-semibold)', marginBottom: 'var(--space-1)' }}>{schedule.name}</div><div style={{ fontSize: 'var(--font-size-xs)', color: 'var(--text-muted)' }}>{formatDate(schedule.date, 'date')} at {schedule.time} • {schedule.duration} min • {schedule.area}</div></div>
                  <div style={{ display: 'flex', gap: 'var(--space-2)', alignItems: 'center' }}>
                    <span className={`badge ${schedule.priority === 'High' ? 'badge-error' : schedule.priority === 'Medium' ? 'badge-warning' : 'badge-info'}`}>{schedule.priority}</span>
                    <span className="badge badge-success">{schedule.status}</span>
                    <button onClick={() => handleDeleteSchedule(schedule.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 'var(--space-1)', color: 'var(--text-muted)', fontSize: 'var(--font-size-lg)', lineHeight: 1, opacity: 0.7 }} title="Delete schedule">×</button>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="empty-state">
              <div className="empty-state-icon"><Calendar size={48} strokeWidth={1} aria-hidden /></div>
              <div className="empty-state-title">No Scheduled Flights</div>
              <div className="empty-state-description">Create a new flight schedule to get started</div>
            </div>
          )}
        </div>
        ...
        <div className="card card-elevated animate-fade-in-up stagger-3">
          ...
          {flightHistory.length > 0 ? (
            <div className="list">
              {flightHistory.map((flight) => (
                <div key={flight.id} className="list-item">
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 'var(--font-size-sm)', fontWeight: 'var(--font-weight-semibold)', marginBottom: 'var(--space-2)' }}>Flight #{flight.id}</div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 'var(--space-2)', fontSize: 'var(--font-size-xs)', color: 'var(--text-muted)' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-1)' }}><Calendar size={12} aria-hidden /> {formatDate(flight.date, 'date')}</div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-1)' }}><Clock size={12} aria-hidden /> {flight.duration} min</div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-1)' }}><Ruler size={12} aria-hidden /> {flight.distance} km</div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-1)' }}><ImageIcon size={12} aria-hidden /> {flight.images} images</div>
                    </div>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-1)', alignItems: 'flex-end' }}>
                    <span className="badge badge-success">{flight.status}</span>
                    <div style={{ fontSize: 'var(--font-size-xs)', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: 'var(--space-1)' }}>
                      <Battery size={12} aria-hidden /> {flight.batteryStart}% → {flight.batteryEnd}%
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="empty-state">
              <div className="empty-state-icon"><Activity size={48} strokeWidth={1} aria-hidden /></div>
